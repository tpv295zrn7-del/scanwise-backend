...
app.post('/api/scans', async (req, res) => {
  ...
  if (barcode && !image) {
    ...
    // Fall back to Open Food Facts for any real-world barcode not in
    // the local seed. Without this, every scan the user does returns
    // an empty product and the Compare button has no name/brand to pass.
    let offProduct = null;
    let offError = null;
    try {
      const offRes = await fetch(
        `https://world.openfoodfacts.org/api/v2/product/${encodeURIComponent(barcode)}.json`,
        { headers: { 'User-Agent': 'ScanWise/1.0 (contact@scanwise.app)' } }
      );
      if (offRes.ok) {
      ...
      offError = e.message;
      console.error('Open Food Facts fallback failed:', e.message);
    }
    if (offProduct) {
      // Best-effort analytics write. Don't fail the response if the DB
      // happens to be read-only — the product data is the important part.
      try {
        db.prepare('INSERT INTO scans (barcode, confidence) VALUES (?, ?)').run(barcode, 'estimated');
      } catch (e) {
        console.warn('Could not write scan row:', e.message);
      }
      return res.json({
        found: true,
        product: offProduct,
...
function handleAlternatives({ barcode, category, name, goals, res }) {
  ...
  const scored = sameCategory.map(p => ({
    ...
    // Seed barcodes are synthetic (e.g. 0012345678901) so Open Food
    // Facts won't have product images for them. Generate a
    // deterministic placeholder URL keyed off the product name so
    // the mobile can show a labelled tile per alternative.
    image: alternativeImageUrl(p)
  }));
  ...
  res.json({
    ...
    goals,
    alternatives: top3.map((alt) => ({
      ...alt,
      // Re-attach the image here so it's always present in the
      // response, even if the upstream map got it stripped for any
      // reason.
      image: alternativeImageUrl(alt)
    })),
    total_considered: sameCategory.length
  });
}

// Generate a placeholder image URL for a seed alternative. The seed uses
// synthetic barcodes so Open Food Facts has no image for them. We
// pick a color from the category and put the product name on the tile.
function alternativeImageUrl(product) {
  const colors = {
    cereal: '4F46E5',   // indigo
    snack: 'F59E0B',    // amber
    yogurt: 'EC4899',   // pink
    drink: '06B6D4',    // cyan
    lunchbox: '10B981', // emerald
    default: '6B7280'   // gray
  };
  const color = colors[product.category] || colors.default;
  const text = encodeURIComponent((product.name || 'Product').slice(0, 40));
  return `https://placehold.co/400x400/${color}/ffffff/png?text=${text}`;
}
